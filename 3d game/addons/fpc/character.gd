...
@export var CROUCH : String = "shift"
...
func _physics_process(delta):
	...
	var fan_speed = (velocity.x**2+velocity.y**2)**0.5
	if fan_speed > 0.3 and audioo.playing != true:
		audioo.playing = true
	elif fan_speed <= 0.3:
		audioo.playing = false




@onready var audioo = $AudioStreamPlayer3D2
...
@onready var fonar = $Head/Camera/SpotLight3D


func _process(delta):
	
	if Input.is_action_just_pressed("f"):
		if fonar.visible == true:
			fonar.visible = false
		elif fonar.visible == false:
			fonar.visible = true
	...
	# Uncomment if you want full controller support
	#var controller_view_rotation = Input.get_vector(LEFT, RIGHT, FORWARD, BACKWARD)
